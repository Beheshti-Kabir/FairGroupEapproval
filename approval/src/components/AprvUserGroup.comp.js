...
export default class AprvUserGroup extends Component {
    constructor(props) {
        super(props);
    ...
    retrieveObjs = () => {
        ...
        AprvUserGroupService.getAll(params)
                .then((res) => {
                    //  alert(JSON.stringify(res.data));
...
//                        var nam = this.state.authUsers.filter(x => x.value === onLine.authUser)[0].label;
//
...
//this.setState({currObj: xxx});
    ...
    deleteObj = () => {
        this.setState({loading: true});
        AprvUserGroupService.delete(this.state.currObj.id)
                .then(res => {
                    this.setState({
                        loading: false,
    ...
    clear = () => {
        this.setState({
            message: '',
            currObj: {name: '', description: '', active: false, lines: []}
        });
    }
